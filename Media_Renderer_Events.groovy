 ...
 *  You can add your key in line 99 to avoid writing by the app
 *  Date: 2015-10-09
 */
definition(
...
preferences {
	page(name: "mainPage", title: "Play a message on your MediaRenderer or Sonos  when something happens", install: true, uninstall: true)
...
def mainPage() {
	dynamicPage(name: "mainPage") {
		...
		section{
            input "actionType", "enum", title: "Action?", required: true, defaultValue: "Message",submitOnChange:true, options: ["Message","Sound","Track","Radio Tunes"]
        ...
        section{  
	        ...
	        input "radioTunes", "enum", title: "Play this RadioTunes Station?", required: true, defaultValue: "hit00s", options: radioTunesOptions
		}
        section("Radio Tunes settings", hideable: actionType == "Radio Tunes" && !RTKey ? flase:true, hidden: true) {
        	input "RTKey","text",title:"Radio Tunes Key?", required:actionType == "Radio Tunes" ? true:flase
    ...
    switch(actionType) {
		...
		case "Track":
         ...
         case "Radio Tunes":
            ...
            //	sonos.playTrackAndResume(state.soundErrorRadioTunes.uri, state.soundErrorRadioTunes.duration, volume)
            }
            break
	}
