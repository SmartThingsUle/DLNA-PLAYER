...
definition(
	...
	description: "Allows you to control your Media Render from the SmartThings app. Perform basic functions like play,  stop, change track, and check artist and song name from the Things screen.",
...
preferences {
	page(name:"mediaRenderDiscovery", title:"Media Render Device Setup", content:"mediaRenderDiscovery", refreshTimeout:5)
}
//PAGES
def mediaRenderDiscovery()
{
	if(canInstallLabs())
	{
		int mediaRenderRefreshCount = !state.mediaRenderRefreshCount ? 0 : state.mediaRenderRefreshCount as int
		state.mediaRenderRefreshCount = mediaRenderRefreshCount + 1
		def refreshInterval = 3

		def options = mediaRenderesDiscovered() ?: []
		...
		//mediaRender discovery request every 5 //25 seconds
		if((mediaRenderRefreshCount % 8) == 0) {
			discoverMediaRenderes()
		}

		//setup.xml request every 3 seconds except on discoveries
		if(((mediaRenderRefreshCount % 1) == 0) && ((mediaRenderRefreshCount % 8) != 0)) {
			verifyMediaRenderPlayer()
		}

		return dynamicPage(name:"mediaRenderDiscovery", title:"Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true, uninstall: true) {
			section("Please wait while we discover your MediaRender. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
				input "selectedMediaRender", "enum", required:false, title:"Select Media Render (${numFound} found)", multiple:true, options:options
			}
		}
	}
	else
...
To update your Hub, access Location Settings in the Main Menu (tap the gear next to your location name), select your Hub, and choose "Update Hub"."""
		...
		return dynamicPage(name:"mediaRenderDiscovery", title:"Upgrade needed!", nextPage:"", install:false, uninstall: true) {
			section("Upgrade") {
...
private verifyMediaRenderPlayer() {
	def devices = getMediaRenderPlayer().findAll { it?.value?.verified != true }
	
	devices.each {
		verifyMediaRender((it?.value?.ip + ":" + it?.value?.port), it?.value?.ssdpPath)
	}
}

private verifyMediaRender(String deviceNetworkId, String ssdpPath) {
...
Map mediaRenderesDiscovered() {
	def vmediaRenderes = getVerifiedMediaRenderPlayer()
	def map = [:]
	vmediaRenderes.each {
...
def getMediaRenderPlayer()
{
	state.mediaRenderes = state.mediaRenderes ?: [:]
}

def getVerifiedMediaRenderPlayer()
{
	getMediaRenderPlayer().findAll{ it?.value?.verified == true }
}
...
def addMediaRender() {
	def players = getVerifiedMediaRenderPlayer()
	def runSubscribe = false
	selectedMediaRender.each { dni ->
...
def locationHandler(evt) {
	...
	{ //SSDP DISCOVERY EVENTS
		...
		def mediaRenderes = getMediaRenderPlayer()

		
		if (!(mediaRenderes."${parsedEvent.ssdpUSN.toString()}"))
		{ //mediaRender does not exist
			mediaRenderes << ["${parsedEvent.ssdpUSN.toString()}":parsedEvent]
		}
		else
		{ // update the values

			def d = mediaRenderes."${parsedEvent.ssdpUSN.toString()}"
	...
	{ // MEDIARENDER RESPONSES
		...
		{ // description.xml response (application/xml)
			...
			if ( !body?.device?.modelName?.text().startsWith("Sonos") && body?.device?.deviceType?.text().contains("urn:schemas-upnp-org:device:MediaRenderer:1"))
			{
				...
				def mediaRenderes = getMediaRenderPlayer()
				def player = mediaRenderes.find {it?.key?.contains(body?.device?.UDN?.text())}
				if (player)
				{
...
private def parseEventMessage(Map event) {
	//handles mediaRender attribute events
	return event
}
